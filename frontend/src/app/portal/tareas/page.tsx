...
export default function MiPlanPage() {
  ...
  const act = async (fn: () => Promise<any>, stepId: string, successMsg?: string) => {
    setBusy(stepId);
    try { await fn(); await load(); if (successMsg) toast.success(successMsg); }
  ...
  const submitQuiz = async () => {
    ...
    try {
      ...
      setQuizResult(res);
    } catch (e: any) { toast.error(e.message || "No se pudo enviar el cuestionario"); } finally { setSubmitting(false); }
  };
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      {plans.length === 0 ? (
        ...
        return (
          <div key={plan.id} className="bg-[#161e33] border border-[#2a3354] rounded-2xl overflow-hidden">
            <div className="px-6 py-4 border-b border-[#2a3354]">
                        ...
                        <div className="flex items-center gap-1.5 shrink-0">
                          {step.is_running ? (
                            <button onClick={() => act(() => api.pauseStep(step.id), step.id)} disabled={busy === step.id}
                              className="flex items-center gap-1 text-xs bg-[#1c2540] hover:bg-amber-500/10 border border-[#2a3354] hover:border-amber-500/20 text-slate-300 hover:text-amber-400 px-2.5 py-1.5 rounded-xl transition-colors disabled:opacity-50">
                              <Pause size={12} /> Pausar
                            </button>
                          ) : (
                            <button onClick={() => act(() => api.startStep(step.id), step.id)} disabled={busy === step.id}
                              className="flex items-center gap-1 text-xs bg-[#1c2540] hover:bg-indigo-500/10 border border-[#2a3354] hover:border-indigo-500/20 text-slate-300 hover:text-indigo-400 px-2.5 py-1.5 rounded-xl transition-colors disabled:opacity-50">
                              <Play size={12} /> {t > 0 ? "Reanudar" : "Iniciar"}
                            </button>
                          )}
                          <button onClick={() => act(() => api.completeStep(step.id), step.id, "Paso completado")} disabled={busy === step.id}
                            className="flex items-center gap-1 text-xs bg-[#1c2540] hover:bg-emerald-500/10 border border-[#2a3354] hover:border-emerald-500/20 text-slate-300 hover:text-emerald-400 px-2.5 py-1.5 rounded-xl transition-colors disabled:opacity-50">
      ...
      {quiz && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => !submitting && (quizResult ? closeQuiz() : setQuiz(null))}>
          <div className="bg-[#161e33] border border-[#2a3354] rounded-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
                          ...
                          return (
                            <button key={o.id} onClick={() => pickAnswer(q, o.id)}
                              className={`flex items-center gap-2.5 text-left px-3 py-2 rounded-xl border text-sm transition-colors
                                ${sel ? "bg-indigo-500/10 border-indigo-500/40 text-white" : "bg-[#0f1629] border-[#2a3354] text-slate-300 hover:border-[#3a4a72]"}`}>
                              <span className={`w-4 h-4 ${q.qtype === "single" ? "rounded-full" : "rounded-md"} border flex items-center justify-center shrink-0
                                ${sel ? "bg-indigo-500 border-indigo-500" : "border-[#2a3354]"}`}>
